...
class AFD:
    ...
    def add_transition(self, from_state: str, symbol: str, to_states: List[str]):
        if from_state in self.states and all(to_state in self.states for to_state in to_states) and symbol in self.alphabet:
            if from_state not in self.transitions:
                self.transitions[from_state] = {}
            if symbol not in self.transitions[from_state]:
                self.transitions[from_state][symbol] = []
            self.transitions[from_state][symbol].extend(to_states)
            
    def validate_string(self, input_string: str) -> Tuple[bool, List[Tuple[str, str, List[str]]]]:
        ...
        for char in input_string:
            ...
            trace.append((current_states, char, next_states))
            current_states = next_states

        return any(state in self.acceptance_states for state in current_states), trace
...
def main(page: ft.Page):
        ...
        for state in sorted(afd.states):
            ...
            for symbol in sorted(afd.alphabet):
                ...
                if state in afd.transitions and symbol in afd.transitions[state]:
                    current_value = ",".join(afd.transitions[state][symbol])
    ...
    def update_transitions(e):
        for (state, symbol), text_field in transition_fields.items():
            to_states = [s.strip() for s in text_field.value.split(",") if s.strip()]
            if to_states:
                if all(to_state in afd.states for to_state in to_states):
                    afd.add_transition(state, symbol, to_states)
                else:
                    text_field.value = ""
                    text_field.border_color = ft.colors.RED
                    text_field.hint_text = "Estado(s) inválido(s)"
                    page.update()
    ...
    # Display trace
        ...
        # Header for the trace table
            trace_header = ft.Row([
                ft.Text("Estados Actuales", width=200, weight=ft.FontWeight.BOLD),
                ft.Text("Símbolo", width=100, weight=ft.FontWeight.BOLD),
                ft.Text("Estados Siguientes", width=200, weight=ft.FontWeight.BOLD)
            ])
            simulation_trace.controls.append(trace_header)
        
        # Add each step in the trace
            for current_states, symbol, next_states in trace:
                step = ft.Row([
                    ft.Text(", ".join(current_states), width=200),
                    ft.Text(symbol, width=100),
                    ft.Text(", ".join(next_states), width=200)
                ])
